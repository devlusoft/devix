...
export function devix(config: DevixConfig): UserConfig {
  ...
  const pkgRoot = dirname(_require.resolve('@devlusoft/devix/package.json'))

  const renderPath = resolve(pkgRoot, 'dist/server/render.js').replace(/\\/g, '/')
  const apiPath = resolve(pkgRoot, 'dist/server/api.js').replace(/\\/g, '/')
  const actionsPath = resolve(pkgRoot, 'dist/server/actions.js').replace(/\\/g, '/')
  const matcherPath = resolve(pkgRoot, 'dist/runtime/client-router.js').replace(/\\/g, '/')
  const routesPath = resolve(pkgRoot, 'dist/server/routes.js').replace(/\\/g, '/')
  const envPath = resolve(pkgRoot, 'dist/utils/env.js').replace(/\\/g, '/')
